...
>
> 📌 **Lệnh dài vì máy khám nằm trong gói plugin, không nằm trong thư mục bạn đang đứng.** `${CLAUDE_PLUGIN_ROOT}` là biến Claude Code tự đặt — cứ dán nguyên văn, đừng thay bằng đường dẫn tay. Câu trả lời của bạn thì ghi vào **kho của bạn** (`kham-ra/phien/`), không ghi vào gói: gói bị thay mỗi lần `claude plugin update`.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" o <phiên>
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" moi
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" dap <phiên> 3                  # ô bấm chọn — ghi số thứ tự
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" dap <phiên> 3,6                # ô chọn nhiều
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" dap <phiên> 12                 # ô đếm số
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" dap <phiên> "kể nguyên văn…"   # ô kể bằng lời
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" bo  <phiên>                    # bí, bỏ qua
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" lui <phiên>                    # sửa câu vừa rồi
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" quet <phiên>
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" quet <phiên>    # đọc lại toàn bộ tín hiệu
node "${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/kham.mjs" soan <phiên>    # sinh kham-ra/<…>/de-xuat.md
```

Rồi **đọc `${CLAUDE_PLUGIN_ROOT}/skills/kham-benh/bo-kham/luat-ke-don.md` TRỌN VẸN** và kê đơn theo đúng luật trong đó. Đọc kèm các trang bản khám tự liệt kê ở dòng *"Trang wiki bắt buộc đọc"*.
...
Lệnh đó đọc chính bản khám vừa lưu, hỏi con số hiện tại và chốt mục tiêu sau ba tháng, hẹn ngày tái khám, rồi mỗi tháng tập trung chữa **một quy trình** trong khâu này: cùng họ lập **việc mỗi ngày**, đi đủ năm bước **làm thật → đóng gói → tối ưu → đơn giản → tự động**, cuối tháng nghiệm thu. Hết 90 ngày, nó mời quay lại đây tái khám.
...
Giữa hai lần khám là **ba tháng chữa** — mỗi tháng một quy trình, mỗi ngày một việc — do `/dieu-hanh` điều. Khám một lần chỉ nói được một câu. Khám bốn lần trong mười hai tháng thì thành **đường tiến của từng khâu, đo được** — đó mới là chỗ bộ máy này đáng tiền. Lượt sau máy tự hỏi tái khám và tách ba lý do chưa đỡ: **chưa làm** · **làm rồi mà không ăn thua** · **chẩn nhầm từ đầu**. Ba thứ đó chữa khác hẳn nhau.
